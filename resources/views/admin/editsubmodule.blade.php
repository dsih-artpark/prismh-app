      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header pb-0">
            <h5>Edit Sub Module</h5>
          </div>
          <div class="card-body add-post">
            <form class="row needs-validation " novalidate="" method="post" action="{{route('admin.submodule.update', ["id"=>$submodule['det']->id])}}" enctype='multipart/form-data'>
                @csrf
                @method('PUT')
              <div class="col-sm-12">
                <div class="mb-3">
                  <label for="validationCustom01">Title</label>
                  <input class="form-control" id="validationCustom01" type="text" name="name" placeholder="Add Sub Module Title" value="{{$submodule['det']->name}}" required>
                </div>
                
                <div class="mb-3">
                          <label class="form-label" for="validationCustom04">Modules</label>
                          <select class="form-select" id="validationCustom04" name="mod_id" required="" >
                            <option selected disabled="" value="">Choose any one</option>
                            @foreach($modules as $cat)
                             @php 
                             $categorysel = ($cat->id == $submodule['det']->mod_id)?"selected":"";
                             @endphp
                ...
                <div class="mb-3">
                  <div class="media">
                    ...
                    <div class="media-body text-end">
                      <label class="switch">
                          @php
                          $chk = $submodule['det']->status == '1' ? "checked" : " ";
                          @endphp
