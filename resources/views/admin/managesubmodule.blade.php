          ...
          <div class="card-body">
            <div class="media static-widget">
              <div class="media-body">
                <h6 class="font-roboto">Sub Modules</h6>
                @php
                $total = DB::table('submodules')->count();
                @endphp
          ...
          <div class="card-body">
            <div class="media static-widget">
              <div class="media-body">
                ...
                @php
                $data = DB::table('submodules')->where('status', 1)->count();
                @endphp
          ...
          <div class="card-body">
            <div class="media static-widget">
              <div class="media-body">
                ...
                @php
                $data = DB::table('submodules')->where('status', 2)->count();
                @endphp
                <h4 class="mb-0 counter">{{$data}}</h4>
              </div>
        ...
        <div class="card">
          <div class="card-header pb-0">
            ...
            <div class="mb-3 text-end">
              <a href="{{route('adminsubmodule.add')}}">
                <button class="btn btn-square btn-primary" type="button" data-bs-original-title="" title="">Add Sub Module</button>
              </a>
            </div>
          </div>
          <div class="card-body">
            <div class="table-responsive">
              ...
              <table class="display" id="submodule" style="width:100%">
                <thead>
                  <tr>
                    <th>#</th>
                    <th>Sub Module</th>
                ...
                <tbody>
                    @php $z = 1; @endphp
  @foreach($submodules as $role)  
                    <tr>
                        <td>{{$z}}</td>
                        <td>{{$role->name}}</td>
                        <td>
                            @php
                $moduledet = DB::table('modules')->where('id',$role->mod_id)->first();
                @endphp
                {{$moduledet->name}}
                        </td>
                        <td>
...
<a href="{{route('admin.submodule.edit' , ['id' => $role->id])}}" class="btn btn-outline-primary-2x"><i class="icon-pencil-alt"></i></a>

<a href="{{route('admin.submodule.delete' , ['id' => $role->id])}}" class="btn btn-outline-danger-2x" ><i class="icon-trash" ></i></a>
                        </td>
                    </tr>
                    @php $z++; @endphp
                    @endforeach
