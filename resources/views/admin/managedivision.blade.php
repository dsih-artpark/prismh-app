...
<div class="page-body">
          ...
          <div class="card-body">
            <div class="media static-widget">
              <div class="media-body">
                <!--<h6 class="font-roboto">Division</h6>-->
                
                <h6 class="font-roboto">Divisions</h6>
                @php
                $total = DB::table('division')->count();
                @endphp
          ...
          <div class="card-body">
            <div class="media static-widget">
              <div class="media-body">
                ...
                @php
                $data = DB::table('division')->where('status', 1)->count();
                @endphp
          ...
          <div class="card-body">
            <div class="media static-widget">
              <div class="media-body">
                ...
                @php
                $data = DB::table('division')->where('status', 2)->count();
                @endphp
                <h4 class="mb-0 counter">{{$data}}</h4>
              </div>
        ...
        <div class="card">
          <div class="card-header pb-0">
            ...
            <div class="mb-3 text-end">
              <a href="{{route('admindivision.add')}}">
          ...
          <div class="card-body">
            <div class="table-responsive">
              ...
              <table class="display" id="division" style="width:100%">
                <thead>
                  <tr>
                    <th>#</th>
                    <th>Zone</th>
  ...
  @foreach($divisions as $role)  
                    <tr>
                        ...
                        <td>
                            @php
                $moduledet = DB::table('zone')->where('id',$role->zone_id)->first();
                @endphp
                {{$moduledet->title}}
                        </td>
                        <td>{{$role->name}}</td>
                        <td>{{$role->latitude}}</td>
                        <td>{{$role->longitude}}</td>
                        <td>
                             @if($role->status == 1)
...
<a href="{{route('admin.division.edit' , ['id' => $role->id])}}" class="btn btn-outline-primary-2x"><i class="icon-pencil-alt"></i></a>

<a href="{{route('admin.division.delete' , ['id' => $role->id])}}" class="btn btn-outline-danger-2x" ><i class="icon-trash" ></i></a>
                        </td>
                    </tr>
                    @php $z++; @endphp
                    @endforeach
