      ...
      <div class="col-sm-12">
        <div class="card">
          ...
          <div class="card-body add-post">
            <form class="row needs-validation " novalidate="" method="post" action="{{route('admin.division.update', ["id"=>$division['det']->id])}}" enctype='multipart/form-data'>
                @csrf
                @method('PUT')
              <div class="col-sm-12">
                <div class="mb-3">
                          <label class="form-label" for="validationCustom04">Zones</label>
                          <select class="form-select" id="validationCustom04" name="zone_id" required="" >
                            <option selected disabled="" value="">Choose any one</option>
                            @foreach($zones as $cat)
                             @php 
                             $categorysel = ($cat->id == $division['det']->zone_id)?"selected":"";
                             @endphp
                            <option value="{{$cat->id}}" <?=$categorysel;?>>{{$cat->title}}</option>
                            @endforeach
                          </select>
              ...
              <div class="mb-3">
                  ...
                  <input class="form-control" id="validationCustom01" type="text" name="name" placeholder="Add Division Title" value="{{$division['det']->name}}" required>
                </div>
                 <div class="mb-3">
                  <label for="validationCustom01">Latitude</label>
                  <input class="form-control" id="validationCustom01" type="text" name="latitude" placeholder="Add Latitude" value="{{$division['det']->latitude}}" required>
                </div>
                 <div class="mb-3">
                  <label for="validationCustom01">Longitude</label>
                  <input class="form-control" id="validationCustom01" type="text" name="longitude" placeholder="Add Longitude" value="{{$division['det']->longitude}}" required>
                </div>
                <div class="mb-3">
                  <div class="media">
                    ...
                    <div class="media-body text-end">
                      <label class="switch">
                          @php
                          $chk = $division['det']->status == '1' ? "checked" : " ";
                          @endphp
