          ...
          <div class="card-body">
            <div class="media static-widget">
              <div class="media-body">
                ...
                @php
                $data = DB::table('ward')->where('status', 1)->count();
                @endphp
          ...
          <div class="card-body">
            <div class="media static-widget">
              <div class="media-body">
                ...
                @php
                $data = DB::table('ward')->where('status', 2)->count();
                @endphp
                <h4 class="mb-0 counter">{{$data}}</h4>
              </div>
        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="table-responsive">
              ...
              <table class="display" id="ward" style="width:100%">
                <thead>
                  <tr>
                    <th>#</th>
                    <th>Division</th>
                ...
                <tbody>
                    @php $z = 1; @endphp
  @foreach($wards as $role)  
                    <tr>
                        <td>{{$z}}</td>
                        <td>
                            @php
                $moduledet = DB::table('division')->where('id',$role->division_id)->first();
                @endphp
                {{$moduledet->name}}
                        </td>
                        <td>{{$role->name}}</td>
...
<a href="{{route('admin.ward.edit' , ['id' => $role->id])}}" class="btn btn-outline-primary-2x"><i class="icon-pencil-alt"></i></a>

<a href="{{route('admin.ward.delete' , ['id' => $role->id])}}" class="btn btn-outline-danger-2x" ><i class="icon-trash" ></i></a>
                        </td>
                    </tr>
                    @php $z++; @endphp
                    @endforeach
