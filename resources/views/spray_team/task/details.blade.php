...
@section('content')
  <div class="page-content" style="padding-bottom: 0px; transform: translateX(0px);">
    @include('spray_team.layout.header', ['title' => 'Task Details', 'back' => true])
    ...
    <div class="card card-style">
      ...
      <div class="content mb-0">
        ...
        <div class="p-2">
          <span class="">{{$task->uid}}</span><br>
          <span class="">{{$task->latit}}</span><br>
          @if($task->q1)
            <span class="">Does Breeding Spots Exist ? {{$task->q1}}</span>
          @endif
          @if($task->q1 == "Yes")
          - <span class="">{{$task->waste}}</span>
          @endif
          <br>
          <span class="">{{$task->descp}}</span>
          <br>
          <span class="">{!! date('d-m-Y', strtotime($task->created_at)) !!}</span>
          <br>
          <span class="">{!! date('h:i a', strtotime($task->created_at)) !!}</span>
          <br><br>
        </div>
      </div>
    </div>
