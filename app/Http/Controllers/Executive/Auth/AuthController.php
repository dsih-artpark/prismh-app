...
class AuthController extends Controller
{
  ...
  protected $redirectTo = '/executive/dashboard';
  ...
  {
    if($request->ajax()){
      $admin =  Customer::where('phone', $request->input('phone'))->whereIn('roles', [3,4])->first();
    ...
    return view('executive.otp_login');
  ...
  public function login(Request $request)
  {
    if($request->has('type') AND $request->type == 'otp_login'){
      ...
      $check = Customer::where('email', $request->email)->whereIn('roles', [3,4])->first();
      if(!$check)
